		...
		<div id="content">
			...
			<div class="panel box-shadow-none content-header">
				<div class="panel-body">
					<div class="col-md-12">
						<h3 class="animated fadeInLeft">Lista Programas</h3>
					</div>
				</div>
			</div>
					...
					<div class="panel">
						<div class="panel-heading">
							<div style="position:absolute; right:30px">
								<form action = "<%=request.getContextPath()%>/programa">
									<input type="hidden" name="opcion" value="crear">
									<button class="btn btn-success">Crear Programa</button>
								</form>
							</div>
							<h4>Programas</h4>
						</div>
									...
									<div class="row">
										<div class="col-sm-12">
											<table id="datatables-example" class="table table-striped table-bordered dataTable no-footer" width="100%" cellspacing="0" role="grid" aria-describedby="datatables-example_info" style="width: 100%;">
												...
												<tbody>
													<c:if test="${not empty listaProgramas}">
														<c:forEach items="${listaProgramas}" var="programa">
															<tr role="row">
																<td class="sorting_1" style="text-align:center;">${programa.nombre }</td>
																
																<td style="text-align:center;">${programa.descripcion }</td>
																
																<td style="width:10%">
																	<form action = "<%=request.getContextPath()%>/programa">
																		<input type="hidden" name="opcion" value="modificar">
																		<button name="id_programa" class="btn btn-warning" value=${programa.id }>Modificar</button>
																	</form>	
																</td>
																<td style="width:10%">
																	<form>
																		<input type="hidden" name="opcion" value="eliminar">
																		<button onclick="if (confirm('Seguro que quieres eliminar el programa: ${programa.nombre }?')) { form.action = '<%=request.getContextPath()%>/programa'; } else { return false; }" name="id_programa" class="btn btn-danger" value=${programa.id }>Eliminar</button>
																	</form>
																</td>
															</tr>
														</c:forEach>
